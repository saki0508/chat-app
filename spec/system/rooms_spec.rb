...
RSpec.describe "チャットルームの削除機能", type: :system do
  before do
    @room_user = FactoryBot.create(:room_user) #roomとuserが同時生成（中間テーブルでアソシエーションしてるから）
  end

  it 'チャットルームを削除すると、関連メッセージが全て削除されていること' do
    # サインインする
    ...
    # 「チャットを終了する」ボタンをクリックすると、作成した５つのメッセージが削除されていることを期待する
    expect{
